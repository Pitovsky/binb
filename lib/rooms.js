...
Room.prototype.addPointsAndStats = function(nickname, allinone) {
  ...
  userData.guessed++;
  userData.guesstime = songTime - this.songtimeleft;
...
Room.prototype.sendLoadTrack = function() {
  ...
  songsdb.zrange([this.roomname, index, index], function(err, res) {
    ...
    songsdb.hmget([
      ...
      room.artistName = replies[0] == null ? 'null' : replies[0];
      room.artist = room.artistName.toLowerCase();
      room.trackName = replies[1] == null ? 'null' : replies[1];
      ...
      primus.room(room.roomname).send('loadtrack', {url: room.previewUrl, startTime: Math.random() * 35 + 5});
...
Room.prototype.sendPlayTrack = function() {
  ...
  this.startTimer(Date.now() + songTime, 50);
  setTimeout(this.sendTrackInfo.bind(this), songTime);
};
...
Room.prototype.sendTrackInfo = function() {
  primus.room(this.roomname).send('trackinfo', {
    ...
    trackInitTime: Math.random() * 35 + 5
  });
