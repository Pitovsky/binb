...
var normalize = function(text) {
  ...
  text = text.replace(/( & | and | \+ | и | feat | feat. | ft. | ft | vs | vs. | amp; )/g, ', ');

  text = text.replace(/(\([^\)]*\)|\[[^\]]*\])/g, ''); //remove (inner)
  text.replace(/[\[\]\(\)]/g, ''); //remove elapsed )))))

  //remove info like 'Scorpions / Скорпионс' and 'Something - song for mobile phones'
  if (~text.indexOf(' -')) {
...
var artistNormalize = function(artist) {
...
var titleNormalize = function(title) {
  title = title.replace(/[,]/g, '');
  title = title.replace(/(remix|bonus|original)/g, '');
  return title.trim();
};
...
module.exports = function(pattern, guess, enableartistrules) {
  ...
  if (enableartistrules) {
    ...
    for (var pi = 0; pi < patterns.length; pi++) {
      for (var gi = 0; gi < guesses.length; gi++) {
        if (checkDistance(artistNormalize(patterns[pi]), artistNormalize(guesses[gi]))) {
          return true;
        }
      }
    }
    ...
    return checkDistance(titleNormalize(pattern), titleNormalize(guess));
  }
};
  ...
  if (~pattern.indexOf(' и ')) {
    // Allow to write "and" in place of " и "
    _pattern = _pattern.replace(/ и /, ' and ');
    if (checkDistance(_pattern, guess)) {
      return true;
    }
  }
